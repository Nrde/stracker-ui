...
// xx:yy:zzz -> xxyyzzz
function remTimeSep(mystring){
    mystring = mystring.replace(':','');
    return mystring.replace('.','');
}

// '-' is always sorted to the bottom of the table
...
$(document).ready( function () {
    ...
    // list of columns that don't need sorting
    ...
    $( "#lapstable thead tr th" ).each(function( index ) {
        ...
        if(jQuery.inArray($( this ).text(), doNotSortHeaders) != -1) {
            doNotSortCols.push(index);
        }

        if(jQuery.inArray($( this ).text(), doNotSortByTextHeaders) != -1) {
            doNotSortByTextCols.push(index);
        }
    ...
    var t = $('#lapstable').DataTable({
        ...
        // keep the 0 column always ordered from 0-n
...
% end
    ...
    <div class="row"><div class="col-md-12">
        <table id="lapstable" class="table table-striped table-condensed display compact" style="width:100%">
            <thead>
            <tr>
                <th>#</th>
                <th>Pos</th>
